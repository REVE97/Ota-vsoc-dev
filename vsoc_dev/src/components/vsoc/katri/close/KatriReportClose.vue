      ...
      <article class="table">
        ...
        <table>
          ...
          <tbody v-if="allData.renewal"> <!-- renewal 값이 있을 때 -->
            ...
            <tr>
              ...
              <td colspan="2">
                <table>
                  ...
                  <tbody>
                    <tr v-for="(item, index) in renewalActionList"
                        v-if="renewalActionList && renewalActionList.length > 0">
                      <td>{{ item.actionItem }}</td>
                      <td>{{ item.actionPlanDate }}</td>
                      <td>{{ item.actionContent }}</td>
                      <td>{{ item.actionStatus }}</td>
                      <td class="button">
                        <button class="red line" type="button" @click="removeRenewalAction(item, index)">-</button>
                      </td>
                    </tr>
                    <tr v-else>
            ...
            <tr>
              <th colspan="2">대응/조치 추가</th>
              <td colspan="2">
                <div class="group">
                  <div class="input">
                    <input type="text" placeholder="대응/조치 항목을 입력하세요" v-model="renewalActionInput.actionItem" />
                  </div>
                  <div class="input calendar">
                    <Datepicker
                  ...
                  <div class="input">
                    <input 
                      ...
                      v-model="renewalActionInput.actionContent" />
                  </div>
                  <div class="select">
                    <select v-model="renewalActionInput.actionStatus">
                  ...
                  <div class="button">
                    <button type="button" @click="addRenewalAction">+</button>
                  </div>
                </div>
              </td>
            </tr>
            ...
            <tr>
              ...
              <td colspan="5">
                <table>
                  ...
                  <tbody>
                    <tr 
                      v-for="(item, index) in renewalActionList"
                      v-if="renewalActionList && renewalActionList.length > 0">
                      <td>{{ item.actionItem }}</td>
                      <td>{{ item.actionPlanDate }}</td>
                      <td>{{ item.actionContent }}</td>
                ...
                <div class="group">
                  <div class="input">
                    <input 
                      ...
                      v-model="renewalActionInput.actionItem" />
                  </div>
                  <div class="input calendar">
                    <Datepicker
                      v-model="inputDateTime.actionPlanDate"
...
const requireRenewalActionData = [
  "actionItem",
  "actionPlanDate",
  "actionContent",
  "actionStatus",
...
// 대응/조치 추가 입력용 임시 데이터 상태값
const renewalActionInput = ref({
  actionItem: "",
  actionPlanDate: "",
  actionContent: "",
  actionStatus: "",
...
// 화면에 보여주고 최종 POST 할 대응/조치 계획 리스트
const renewalActionList = ref([])
...
const isAllInput = computed(() => {
  ...
  const inputBaseValid = requireData.every((key) => {
    const value = postData.value[key];
    return value !== null && value !== undefined && value !== "";
  })

  const inputRenewalValid = requireRenewalData.every((key) => {
    const value = postData.value.renewal[key];
    return value !== null && value !== undefined && value !== "";
  })

  return inputBaseValid && inputRenewalValid;
})
...
// 대응/조치 계획 + 후에 다시 초기화
const resetRenewalActionInput = () => {
  renewalActionInput.value = {
    actionItem: "",
    actionPlanDate: "",
    actionContent: "",
    actionStatus: "",
  };
  inputDateTime.value.actionPlanDate = "";
  inputDateTime.value.actionPlanTime = "";
};

// 날짜 + 시간 POST 포맷팅 파싱
const formatDateTimeForOracle = (dateValue, timeValue = "") => {
  if (!dateValue) return "";

  const normalizedDate = String(dateValue).trim();
  let normalizedTime = String(timeValue ?? "").trim();
...
// 날짜 + 시간 출력 포맷팅 파싱
const normalizeActionPlanDate = (value) => {
  if (!value) return "";
  ...
  if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/.test(strValue)) {
    return `${strValue}:00`;
  }
...
// 대응/조치 계획 리스트 반영
const syncRenewalActionToPostData = () => {
  postData.value.renewal.renewalAction = renewalActionList.value.map(({ isNew, ...item }) => ({
    ...item,
    actionPlanDate: normalizeActionPlanDate(item.actionPlanDate),
  }));
};

// 대응/조치 계획 추가 메서드
const addRenewalAction = () => {
  renewalActionInput.value.actionPlanDate = formatDateTimeForOracle(
    inputDateTime.value.actionPlanDate,
    inputDateTime.value.actionPlanTime,
  );

  const isValid = requireRenewalActionData.every((key) => {
    const value = renewalActionInput.value[key];
    return value !== null && value !== undefined && value !== "";
  });

  if (!isValid) {
    alert("대응/조치 추가 항목을 모두 입력하세요");
    return;
  }

  renewalActionList.value.push({
    ...renewalActionInput.value,
    isNew: true,
  });

  syncRenewalActionToPostData();
  resetRenewalActionInput();
};

// 대응/조치 계획 삭제 메서드
const removeRenewalAction = (item, index) => {
  if (item?.id) {
    postData.value.deleteRenewalActionIdList.push(item.id);
  }

  renewalActionList.value.splice(index, 1);
  syncRenewalActionToPostData();
};
...
const getfetch_katri_detail = async() => {
  try {
    ...
    // 기존 대응/조치 계획 리스트 화면 출력 및 삭제 처리를 위해 별도 리스트에 세팅
    renewalActionList.value = Array.isArray(allData.value?.renewal?.renewalAction)
      ? allData.value.renewal.renewalAction.map((item) => ({
          ...item,
          actionPlanDate: normalizeActionPlanDate(item?.actionPlanDate),
          isNew: false,
        }))
...
// KATRI 갱신 데이터 전송 API 호출 (종료)
const postfetch_katri_close = async () => {
  try {
    syncRenewalActionToPostData();
