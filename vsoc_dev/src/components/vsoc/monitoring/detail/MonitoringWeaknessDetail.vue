  ...
  <div class="inner">
    ...
    <section class="page">
      ...
      <article class="group button">
        ...
        <router-link 
          :to="{ name: 'monitoring_weakness_update', params: { weaknessReportId: route.params.weaknessReportId } }">
          <button class="line">수정</button>
        </router-link>
        <button class="cancel" @click='router.push({name: "monitoring_management" })'>목록</button>
        <router-link 
          v-if="isValid?.vulnerabilityReportId === null" 
          :to="{ name: 'monitoring_vulnerability_register', params: { weaknessReportId: isValid.weaknessReportId } }">
...
const openDeleteConfirm = () => {
  store.setModalInfo({
    ...
    rightBtn: async() => { 
      await deletefetch_weakness_detail();
      goList(); 
    },
  })
};
